...
7. Complete each actionable leaf only when its latest verification succeeds and the Uigwe-defined numeric completion guardrails pass. The default is `0.98` for every leaf guardrail, `0.98` for the leaf aggregate, and `0.98` for the run aggregate; selected leaf coverage and success criteria coverage default to `1.00`. Keep the run active after the initial todos close while checking the original goal. Add or replace work for uncovered criteria, preserve replacement links, and reopen completed work when new evidence contradicts it. Unresolved blockers, re-entry requests, or unfinished replacements prevent final completion.
